...
app.utils.filldatable = function(params, paramsMap){
	...
	app.xhr = $.ajax({
		...
		success: function(data) {
			...
			var colNames  = data.aoColumns;
			var listColumns = [];
			var schema = {};
			var ln = colNames.length;
			for (var i=0;i<ln;i++){
				colName = colNames[i].sTitle;
			...
			var rowValues = data.aaData;
			var rowValue;
			var lng = rowValues.length;
			for (var i=0;i<lng;i++){
				rowValue = rowValues[i];
				var gridModel = new app.models.ExportGridModel();
				for(key in rowValue){
				  	var colName = listColumns[key];
...
app.utils.getExportList = function(view, filter, bbox, BBview){
	...
	var url = serverUrl +  "/views/get/" + view + "?filters=" + filter + "&bbox=" + bbox + "&columns=" + displayedColumns ; 
	BBview.url = url;
...
*/
app.utils.initGrid = function(list, VisibleList, col){
