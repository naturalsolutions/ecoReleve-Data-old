var ecoReleveData = (function(app) {
	...
	*/
	...
	function generateListField(node, callback) {
		...
		//var itemlist = $(node).find('itemlist').html();
	...
	function generateFloatField (node, callback) {
		...
		var label = $(node).find('display_label').text();
		// creer le modele champ numerique
		var floatField = new app.models.NumericField({
			id: fieldId,
			name: name,
			display_label: label,
		});
	...
	function generateDateField (node, callback) {
		...
		var label = $(node).find('display_label').text();
		// creer le modele champ numerique
		var dateField = new app.models.DateField({
			id: fieldId,
			name: name,
			display_label: label,
		});
					...
					for (var i = 0; i < len; i++) {
						...
						/*if (url.toLowerCase().indexOf("name_vue") >= 0){
							...
							if (area === undefined){
								...
								if (area === undefined){
									area = data[i].AppModel.DeathStaArea;
								}
							}
						}else{
					...
					for (var i = 0; i < len; i++) {
						...
						/*if (url.toLowerCase().indexOf("name_vue") >= 0){
							...
							if (locality === undefined){
								...
								if (locality === undefined){
									locality = data[i].AppModel.DeathStaLocality;
								}
							}
						}else{
						...
						if (isDatalist) {
							...
							$('<option value=\"' + locality + '\">' + locality + "</option>").appendTo(element);
						}
					}
				}
