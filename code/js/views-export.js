	...
	app.views.ExportColumnsSelection = app.views.BaseView.extend({
		...
		afterRender: function(options) {
			...
			for (var i = 0; i < ln; i++) {
				var field = fieldsList[i];
				var fieldUpper = field.toUpperCase();
				if(this.currentView == 'V_Qry_VIndiv_MonitoredLostPostReleaseIndividuals_LastStations')
				{
					if ((field == "Name_signal_type")||(field == "DATE") || (field == "Frequency") || (field == "Locality") || (field == "RelCaptTag") || (field == "Origin") || (field == "Sex") || (field == "FreqOpti") || (field == "SurveyType@Station") || (field == "MonitoringStatus@Station") || (field.toUpperCase() == "LAT") || (field.toUpperCase() == "LON")){
						app.utils.exportSelectedFieldsList.push(field);
					}
					else{
						fieldsListforFom.push(field);
					...
					if (fieldUpper == "STATION") {
						...
						app.utils.exportSelectedFieldsList.push(fieldUpper);
					} else if (fieldUpper == "SITE_NAME") {
						// si champ station exite, il ne faut pas rajouter ce champ à la liste de champs a afficher
						if (stationAdded === false) {
							app.utils.exportSelectedFieldsList.push(field);
