  ...
  return Backbone.View.extend({
    ...
    initialize: function(options) {
      ...
      if (!options.fieldsets) {
		...
		/*
		debugger;
		var myobj = options.model.get("schema");
		for (prop in myobj){
			fields.push(prop);
...
Form.validators = (function() {
  ...
  validators.errMessages = {
	...
	minval : 'minimum value must be > "{{minval}}"',
	maxval : 'maximum value must be < "{{maxval}}"'
  };
 ...
 // minval implementation
   validators.minval = function(options) {
    ...
    options = _.extend({
      type: 'minval',
      message: this.errMessages.minval
    }, options);
    
    return function minval(value) {
      options.value = value;
      ...
      if (value < options.minval) return err;
    };
  };

   // maxval implementation
   validators.maxval = function(options) {

    if (!options.maxval) throw new Error('Missing required "maxval" options for "maxval" validator');
    
    options = _.extend({
      type: 'maxval',
      message: this.errMessages.maxval
    }, options);
    
    return function maxval(value) {
      options.value = value;
      ...
      if (value > options.maxval) return err;
    };
  };
...
Form.editors = (function() {
   ...
   * PHOTO
   ***/
  editors.Photo = editors.Base.extend({
    ...
    render: function() {
     /*   this.setValue(this.value);
        ...
        return this;*/
		 var $el = Form.helpers.parseHTML(Form.templates.photo({
      }));
  ...
  Form.setTemplates({
	...
	photo: '\
			<div  class="bbf-photo">\
				<a class="button big" data-type="btn-photo" id="capture">photo capture</a>\
			</div>\
	'
  }, {
